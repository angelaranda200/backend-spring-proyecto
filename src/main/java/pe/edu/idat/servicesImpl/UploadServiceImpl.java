...
private final Logger log = LoggerFactory.getLogger(UploadServiceImpl.class);
	...
	public Resource cargar(String nombreFoto) throws MalformedURLException {
		...
		/*if(!recurso.exists() && !recurso.isReadable()) {
			rutaArchivo = Paths.get("src/main/resources/static/images").resolve("no-img.jpg").toAbsolutePath();
	...
	public boolean eliminar(String nombreFoto) {
		if (nombreFoto != null && nombreFoto.length() > 0) {
			Path rutaFotoAnterior = Paths.get("src//main//resources//static/images").resolve(nombreFoto).toAbsolutePath();
